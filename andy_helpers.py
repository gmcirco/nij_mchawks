...
def cal_data(prob, true, data, bins, plot=False, figsize=(6,4), save_plot=False):
    ...
    # Add in some noise if there are ties
    #un_probs = list(pd.unique(cal_dat[prob]))
    ru = np.random.uniform(low=-0.00000001,high=0.00000001,size=cal_dat.shape[0])
    cal_dat[probs] = cal_dat[probs] + ru
    cal_dat['Count'] = 1
