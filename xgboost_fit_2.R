...
# For the final paper
# this is the full evaluation data
...
# Filter out people who recidivated at time 1
...
# full data
xfull <- xgb.DMatrix(label = nij_data$y2, data = xgboost_full)

#test-train datasets
xtrain <- xgb.DMatrix(label = train_data$y2, data = xgboost_train)
xtest <- xgb.DMatrix(label = test_data$y2, data = xgboost_test)
...
xgb_train_1 <- train(
  x = xgboost_train,
  y = as.factor(ifelse(train_data$y2 == 1,"yes","no")),
...
yhat <- predict(fit1.full, xgboost_full)

# Performance
pred <- prediction(yhat, nij_data$y2)
...
cal.plot <- data.frame(y = fct_relevel(as.factor(test_data$y2),"1","0"),
                       yhat = yhat)
...
cal_df <- data.frame(y = test_data$y2,yhat = yhat) %>%
  arrange(yhat) %>%
...
print(bscore <- brier(yhat, test_data$y2, clip = TRUE))
...
xtrain_fin <-
  xgb.DMatrix(label = nij_data$y2, data = xgboost_finaldat)
...
# Export
write_csv(
  data.frame(ID = final_test_data_ID, Probability = final_pred),
  "C:/Users/gioc4/Desktop/final_pred.csv"
)
#################################################
